...
//         profile_pic: user.profile_pic, // agar exist kare
//       })),
//     });
...
export const downloadProfile = async (req,res) =>{
  ...
  console.log("downloadProfile error:", error.message)
  return res.status(500).json({message : error.message})
}
}
...
export const sendConnectionRequest = async (req,res)=>{
  ...
  try{
    ...
    console.log("sendConnectionRequest error:" , error.message)
    return res.status(500).json({message:error.message})
...
export const acceptConnectionRequest = async (req,res)=>{
  ...
  try{
    ...
    if(!connection){
      return res.status(404).json({message:"Conncetion not found"})
    }
